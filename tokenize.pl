...
# word starts with punctuation # don't separate ,, ... -- '' etc.
		...
		if (exists($apo{$word}) # 's morgens
		    || ( $tmp1 eq '(' && $tmp2=~/\)[-A-Za-z]/ ) # (oud-)student, (her)-introductie
		    || ( $tmp1 eq '.' && $tmp2=~/^[0-9]/ ) # kaliber .22
		    || ( $tmp1 eq "'" && $tmp2=~/^[0-9][0-9](-|$)/ ) # '93 '93-'94
		    || ( $tmp1 eq '&' && $tmp2=~/^[A-Za-z]+;/ )
		    || $word=~/^(\.+|,+|-+|`+|'+)$/ # don't split --- etc.
...
# split f5,- hfl50 DM0,50 BEF7,50
         if (($word=~/^([a-zA-Z]+)([0-9][-=0-9.,]*)$/ # not just: F.
             || $word=~/^([a-zA-Z]+)([-=0-9.,]*[0-9])$/)
             && exists($money{$1})) {
	     push(@words2,$1);
	     $word=$2;
